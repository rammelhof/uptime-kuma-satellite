...
class TestGetParamSchema:
    ...
    def test_log_file_has_four_fields(self):
...
# ─── Monitor Type Change Integration Tests ───────────────────────────────────
...
class TestMonitorTypeChange:
    """Integration tests for changing monitor type in the editor dialog.
    
    These tests use Textual's test runner to verify the type change
    workflow works correctly, especially the parameter field rebuild
    that was causing DuplicateIds errors.
    """

    @pytest.fixture
    def mock_main_with_config(self, tmp_path: Path, fresh_registry: None) -> MagicMock:
        """Create a mock main screen with a config containing multiple monitor types."""
        ...
        config.monitors.append(MonitorConfig(
            ...
            params={"host": "localhost", "port": 80, "timeout_seconds": 5},
        ))
        config.monitors.append(MonitorConfig(
            name="ping-monitor",
            monitor_type="ping",
            params={"host": "8.8.8.8", "count": 3, "timeout_seconds": 5},
        ))
        config.monitors.append(MonitorConfig(
            name="log-monitor",
            monitor_type="log_file",
            params={"path": "/var/log/app/error.log", "lookback_minutes": 60, "max_errors": 1},
        ))
    ...
    def test_type_change_file_exists_to_service(self, mock_main_with_config: MagicMock):
        """Test changing from file_exists (1 param) to service (3 params)."""
        mock_main = mock_main_with_config
        
        class TestApp(App):
            def compose(self) -> ComposeResult:
                yield MonitorEditorScreen(mock_main, is_edit=True, edit_index=0)
        ...
        async def run_test():
            ...
            async with app.run_test() as pilot:
                ...
                editor = app.query_one(MonitorEditorScreen)
                
                assert editor._selected_type == "file_exists"
                assert "path" in editor._fields
                assert len(editor._fields) == 1
                
                # Set the Select value using the correct Textual 8.x API
                select = editor.query_one("#type-select", Select)
                select.set_options([("service", "service")])
                select.value = "service"
                await pilot.pause()
                
                assert editor._selected_type == "service"
                assert "host" in editor._fields
                assert "port" in editor._fields
                assert "timeout_seconds" in editor._fields
                assert len(editor._fields) == 3
                assert "path" not in editor._fields

        asyncio.run(run_test())

    def test_type_change_service_to_file_exists(self, mock_main_with_config: MagicMock):
        """Test changing from service (3 params) to file_exists (1 param)."""
        mock_main = mock_main_with_config
        
        class TestApp(App):
            def compose(self) -> ComposeResult:
                yield MonitorEditorScreen(mock_main, is_edit=True, edit_index=1)
        ...
        async def run_test():
            ...
            async with app.run_test() as pilot:
                ...
                select = editor.query_one("#type-select", Select)
                select.set_options([("file_exists", "file_exists")])
                select.value = "file_exists"
                await pilot.pause()
                
                assert editor._selected_type == "file_exists"
                assert "path" in editor._fields
                assert "host" not in editor._fields
                assert "port" not in editor._fields
                assert len(editor._fields) == 1

        asyncio.run(run_test())

    def test_type_change_multiple_times(self, mock_main_with_config: MagicMock):
        """Test changing type multiple times in sequence."""
        mock_main = mock_main_with_config
        ...
        async def run_test():
            ...
            async with app.run_test() as pilot:
                ...
                type_sequence = ["service", "ping", "log_file", "file_exists"]
                expected_param_counts = [3, 3, 4, 1]
                
                for target_type, expected_count in zip(type_sequence, expected_param_counts):
                    select = editor.query_one("#type-select", Select)
                    select.set_options([(target_type, target_type)])
                    select.value = target_type
                    await pilot.pause()
                    assert editor._selected_type == target_type
                    assert len(editor._fields) == expected_count

        asyncio.run(run_test())

    def test_type_change_in_add_mode(self, mock_main_with_config: MagicMock):
        """Test changing type in add mode (no existing params)."""
        mock_main = mock_main_with_config
        
        class TestApp(App):
            def compose(self) -> ComposeResult:
                yield MonitorEditorScreen(mock_main, is_edit=False)
        ...
        async def run_test():
            ...
            async with app.run_test() as pilot:
                await pilot.pause()
                editor = app.query_one(MonitorEditorScreen)
                first_type = MonitorRegistry.list_types()[0]
                assert editor._selected_type == first_type
    ...
    def test_type_change_preserves_name(self, mock_main_with_config: MagicMock):
        """Test that changing type doesn't affect the name input."""
        mock_main = mock_main_with_config
        ...
        async def run_test():
            ...
            async with app.run_test() as pilot:
                ...
                assert name_input.value == "file-monitor"
                
                select = app.screen.query_one("#type-select", Select)
                select.set_options([("service", "service")])
                select.value = "service"
                await pilot.pause()
    ...
    def test_type_change_updates_help_text(self, mock_main_with_config: MagicMock):
        """Test that changing type updates the help text below the type label."""
        mock_main = mock_main_with_config
        ...
        async def run_test():
            ...
            async with app.run_test() as pilot:
                ...
                help_label = app.screen.query_one("#type-help", Label)
                initial_text = help_label.visual.plain
                assert "file exists" in initial_text.lower()
                
                select = app.screen.query_one("#type-select", Select)
                select.value = "service"
                await pilot.pause()
                
                new_text = help_label.visual.plain
                assert "tcp port" in new_text.lower()
...
class TestTUIIntegration:
    ...
    def test_multiple_monitors_in_config(self, config_path: Path):
        ...
        monitors = [
            MonitorConfig(name="m1", monitor_type="file_exists"),
            MonitorConfig(name="m2", monitor_type="service"),
            MonitorConfig(name="m3", monitor_type="ping"),
        ]
