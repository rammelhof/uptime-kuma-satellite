...
PARAM_SCHEMAS: dict[str, list[ParamField]] = {
    ...
    "process_count": [
        ParamField("pattern", "Process Pattern", "text", "python",
                   "Process name or command pattern to search for (matches via pgrep -f / tasklist)"),
        ParamField("min_count", "Min Count", "number", "1",
                   "Minimum number of matching processes expected"),
        ParamField("max_count", "Max Count", "number", "0",
                   "Maximum number of matching processes allowed (0 = no limit)"),
    ],
    "uptime": [
        ParamField("min_uptime_seconds", "Min Uptime (seconds)", "number", "300",
                   "Minimum system uptime in seconds (5 min = 300). Alert if uptime is below this."),
    ],
    "cpu_usage": [
    ...
    "log_file": [
        ...
        ParamField("error_patterns", "Error Patterns (regex)", "text", "(default)",
                   "Comma-separated regex patterns to match. Leave as (default) for built-in patterns: error|exception|fatal|critical."),
    ],
}
...
HELP_TEXTS: dict[str, str] = {
    ...
    "process_count": "Checks that the number of matching processes is within an expected range.",
    "uptime": "Checks that the system has been running for at least N seconds (anti-reboot alert).",
